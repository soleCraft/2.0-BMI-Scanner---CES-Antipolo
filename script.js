...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function decodeData(encodedData) {
        ...
        // Validate decoded data format (example check: ensuring it has the expected number of commas)
        const parts = decodedData.split(',');
        if (parts.length < 9) {
            console.error("Decoded data is not in the expected format:", decodedData);
            return null;
        }
    ...
    async function getDocumentWithRetry(docRef, retries = 3, delay = 500) {
        ...
        throw new Error("Failed to retrieve document after multiple attempts.");
    }
    ...
    async function checkIfStudentExist(studentData) {
        ...
        try {
            const studentDocData = await getDocumentWithRetry(studentDocRef);
            ...
            showAlert("An error occurred while accessing the database. Please try again.");
            return false;
        }
    }
    ...
    async function success(result) {
        ...
        if (!decodedData) {
            showAlert("Invalid QR code format. Please scan a valid QR code.");
            isProcessing = false;
            return;
        }
